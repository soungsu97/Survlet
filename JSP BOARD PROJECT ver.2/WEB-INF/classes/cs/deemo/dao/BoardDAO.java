...
public class BoardDAO {
	...
	public ArrayList<BoardDTO> list(int startRow, int endRow){
		String sql = "select * from (select rownum as num, no, title, writer, context, regdate, views from (select no, title, writer, context, regdate, views from board order by no asc))where num BETWEEN "+ startRow +" and "+endRow;
	...
	public BoardDTO view(String no) {
		...
		try {
			...
			sql ="select title, writer, context, regdate from board where no=?";
			pstmt = con.prepareStatement(sql);
	...
	public boolean insert(BoardDTO dto) {	//DB에 저장이 잘되면 true, 잘안되었으면  false를 반환
		String sql = "insert into board(no, title, writer, context, regdate, views) values (board_ai.NEXTVAL,?,?,?,sysdate,0)"; 
		boolean check = false;
		try {  
	...
	public boolean delete(String no) {
	  ...
	  public ArrayList<BoardDTO> selectSearchBoard(String kwd){
			ArrayList<BoardDTO> dtos = new ArrayList<BoardDTO>();;
